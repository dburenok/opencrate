...
TOP_K           = 24 # Use a number divisible by 2, 3, and 4 for a clean grid
GRID_COLUMNS    = 4  # Number of columns in the results grid

def score_to_color(score: float) -> str:
    """Maps a similarity score (0.2 to 0.8) to an HSL color (red to green)."""
    # Clamp the score to the expected range for color mapping
    score = max(0, min(1, score))
    # Normalize score to a 0-1 range within a typical similarity score band (e.g., 0.2 to 0.8)
    # Scores below 0.2 are red, scores above 0.8 are green.
...
with st.sidebar:
    ...
    st.markdown("Upload an audio file to find the most similar samples from the indexed library.")
...
if "results" not in st.session_state:
    ...
    st.markdown("""
    Results are displayed in a grid, sorted by similarity score (higher is better).
    *Clicking a download button will cause the page to refresh, but the search results are cached and will not be re-computed.*
    """)
